    ...
    public class OracleRepository : IRepository
    {
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string orderField, bool isAsc, int pageSize, int pageIndex) where T : class
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"SELECT COUNT(1) AS Total FROM ({builder.Sql}) AS T;SELECT * FROM (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,X.* FROM ({builder.Sql}) AS X) AS T WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", builder.DynamicParameters, commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string sql, object parameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"SELECT COUNT(1) AS Total FROM ({sql}) AS T;SELECT * FROM (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,X.* FROM ({sql}) AS X) AS T WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", parameter, commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string sql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"SELECT COUNT(1) AS Total FROM ({sql}) AS T;SELECT * FROM (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,X.* FROM ({sql}) AS X) AS T WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", dbParameter.ToDynamicParameters(), commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public (IEnumerable<T> list, long total) FindListByWith<T>(string sql, object parameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"{sql} SELECT COUNT(1) AS Total FROM T;{sql},R AS (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,T.* FROM T) SELECT * FROM R WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", parameter, commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public (IEnumerable<T> list, long total) FindListByWith<T>(string sql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"{sql} SELECT COUNT(1) AS Total FROM T;{sql},R AS (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,T.* FROM T) SELECT * FROM R WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", dbParameter.ToDynamicParameters(), commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public async Task<(IEnumerable<T> list, long total)> FindListAsync<T>(string orderField, bool isAsc, int pageSize, int pageIndex) where T : class
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = await dbConnection.QueryMultipleAsync($"SELECT COUNT(1) AS Total FROM ({builder.Sql}) AS T;SELECT * FROM (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,X.* FROM ({builder.Sql}) AS X) AS T WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", builder.DynamicParameters, commandTimeout: CommandTimeout);
                    var total = await multiQuery?.ReadFirstOrDefaultAsync<long>();
                    var list = await multiQuery?.ReadAsync<T>();
                    return (list, total);
                }
        ...
        public async Task<(IEnumerable<T> list, long total)> FindListAsync<T>(string sql, object parameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = await dbConnection.QueryMultipleAsync($"SELECT COUNT(1) AS Total FROM ({sql}) AS T;SELECT * FROM (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,X.* FROM ({sql}) AS X) AS T WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", parameter, commandTimeout: CommandTimeout);
                    var total = await multiQuery?.ReadFirstOrDefaultAsync<long>();
                    var list = await multiQuery?.ReadAsync<T>();
                    return (list, total);
                }
        ...
        public async Task<(IEnumerable<T> list, long total)> FindListAsync<T>(string sql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = await dbConnection.QueryMultipleAsync($"SELECT COUNT(1) AS Total FROM ({sql}) AS T;SELECT * FROM (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,X.* FROM ({sql}) AS X) AS T WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", dbParameter.ToDynamicParameters(), commandTimeout: CommandTimeout);
                    var total = await multiQuery?.ReadFirstOrDefaultAsync<long>();
                    var list = await multiQuery?.ReadAsync<T>();
                    return (list, total);
                }
        ...
        public async Task<(IEnumerable<T> list, long total)> FindListByWithAsync<T>(string sql, object parameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = await dbConnection.QueryMultipleAsync($"{sql} SELECT COUNT(1) AS Total FROM T;{sql},R AS (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,T.* FROM T) SELECT * FROM R WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", parameter, commandTimeout: CommandTimeout);
                    var total = await multiQuery?.ReadFirstOrDefaultAsync<long>();
                    var list = await multiQuery?.ReadAsync<T>();
                    return (list, total);
                }
        ...
        public async Task<(IEnumerable<T> list, long total)> FindListByWithAsync<T>(string sql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = await dbConnection.QueryMultipleAsync($"{sql} SELECT COUNT(1) AS Total FROM T;{sql},R AS (SELECT ROW_NUMBER() OVER ({orderBy}) AS RowNumber,T.* FROM T) SELECT * FROM R WHERE RowNumber BETWEEN {(pageSize * (pageIndex - 1) + 1)} AND {(pageSize * pageIndex)};", dbParameter.ToDynamicParameters(), commandTimeout: CommandTimeout);
                    var total = await multiQuery?.ReadFirstOrDefaultAsync<long>();
                    var list = await multiQuery?.ReadAsync<T>();
                    return (list, total);
                }
