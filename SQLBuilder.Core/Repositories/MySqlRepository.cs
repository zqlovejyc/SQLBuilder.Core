...
namespace SQLBuilder.Core.Repositories
{
    ...
    public class MySqlRepository : IRepository, IDisposable
    {
        ...
        {
            var result = 0;
            var builder = Sql.Insert<T, object>(() => entity, DatabaseType.MySQL, false);
            if (Transaction?.Connection != null)
            {
        ...
        {
            var result = 0;
            var builder = Sql.Delete<T>(DatabaseType.MySQL).WithKey(keyValue);
            if (Transaction?.Connection != null)
            {
                result = Transaction.Connection.Execute(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var result = 0;
            var builder = Sql.Update<T, object>(() => entity, DatabaseType.MySQL, false).WithKey(entity);
            if (Transaction?.Connection != null)
            {
                result = Transaction.Connection.Execute(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var result = 0;
            var builder = Sql.Update<T, S>(entity, DatabaseType: DatabaseType.MySQL, isEnableNullValue: false).Where(predicate);
            if (Transaction?.Connection != null)
            {
                result = Transaction.Connection.Execute(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var builder = Sql.Select<T, object>(DatabaseType: DatabaseType.MySQL).WithKey(keyValue);
            if (Transaction?.Connection != null)
            {
        ...
        {
            var builder = Sql.Select<T, S>(selector, DatabaseType.MySQL).WithKey(keyValue);
            if (Transaction?.Connection != null)
            {
                return Transaction.Connection.QueryFirstOrDefault<T>(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var builder = Sql.Select<T, object>(DatabaseType: DatabaseType.MySQL).Where(predicate);
            if (Transaction?.Connection != null)
            {
                return Transaction.Connection.QueryFirstOrDefault<T>(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var builder = Sql.Select<T, S>(selector, DatabaseType.MySQL).Where(predicate);
            if (Transaction?.Connection != null)
            {
                return Transaction.Connection.QueryFirstOrDefault<T>(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var builder = Sql.Select<T, object>(DatabaseType: DatabaseType.MySQL);
            if (Transaction?.Connection != null)
            {
        ...
        {
            var builder = Sql.Select<T, S>(selector, DatabaseType.MySQL);
            if (Transaction?.Connection != null)
            {
                return Transaction.Connection.Query<T>(builder.Sql, transaction: Transaction).AsQueryable();
            }
        ...
        {
            var builder = Sql.Select<T, S>(DatabaseType: DatabaseType.MySQL).OrderBy(o => keySelector(o));
            if (Transaction?.Connection != null)
            {
                return Transaction.Connection.Query<T>(builder.Sql, transaction: Transaction);
            }
        ...
        {
            var builder = Sql.Select<T, S>(selector, DatabaseType.MySQL).OrderBy(o => keySelector(o));
            if (Transaction?.Connection != null)
            {
                return Transaction.Connection.Query<T>(builder.Sql, transaction: Transaction);
            }
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string orderField, bool isAsc, int pageSize, int pageIndex) where T : class
        {
            ...
            if (Transaction?.Connection != null)
            {
                var multiQuery = Transaction.Connection.QueryMultiple($"DROP TEMPORARY TABLE IF EXISTS $Temp{guid};CREATE TEMPORARY TABLE $Temp{guid} SELECT * FROM ({builder.Sql}) AS T;SELECT COUNT(1) AS Total FROM $Temp{guid};SELECT * FROM $Temp{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE $Temp{guid};", builder.DynamicParameters, Transaction);
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"DROP TEMPORARY TABLE IF EXISTS $Temp{guid};CREATE TEMPORARY TABLE $Temp{guid} SELECT * FROM ({builder.Sql}) AS T;SELECT COUNT(1) AS Total FROM $Temp{guid};SELECT * FROM $Temp{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE $Temp{guid};", builder.DynamicParameters);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string sql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            if (Transaction?.Connection != null)
            {
                var multiQuery = Transaction.Connection.QueryMultiple($"DROP TEMPORARY TABLE IF EXISTS $Temp{guid};CREATE TEMPORARY TABLE $Temp{guid} SELECT * FROM ({sql}) AS T;SELECT COUNT(1) AS Total FROM $Temp{guid};SELECT * FROM $Temp{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE $Temp{guid};", dbParameter.ToDynamicParameters(), Transaction);
                var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                var list = multiQuery?.Read<T>();
                return (list, total);
            }
        ...
        {
            var builder = Sql.Select<T, object>(DatabaseType: DatabaseType.MySQL).OrderBy(o => keySelector(o));
            if (Transaction?.Connection != null)
            {
                return await Transaction.Connection.QueryAsync<T>(builder.Sql, transaction: Transaction);
            }
        ...
        public async Task<(IEnumerable<T> list, long total)> FindListAsync<T>(string orderField, bool isAsc, int pageSize, int pageIndex) where T : class
        {
            ...
            if (Transaction?.Connection != null)
            {
                var multiQuery = await Transaction.Connection.QueryMultipleAsync($"DROP TEMPORARY TABLE IF EXISTS $Temp{guid};CREATE TEMPORARY TABLE $Temp{guid} SELECT * FROM ({builder.Sql}) AS T;SELECT COUNT(1) AS Total FROM $Temp{guid};SELECT * FROM $Temp{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE $Temp{guid};", builder.DynamicParameters, Transaction);
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = await dbConnection.QueryMultipleAsync($"DROP TEMPORARY TABLE IF EXISTS $Temp{guid};CREATE TEMPORARY TABLE $Temp{guid} SELECT * FROM ({builder.Sql}) AS T;SELECT COUNT(1) AS Total FROM $Temp{guid};SELECT * FROM $Temp{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE $Temp{guid};", builder.DynamicParameters);
                    var total = await multiQuery?.ReadFirstOrDefaultAsync<long>();
                    var list = await multiQuery?.ReadAsync<T>();
                    return (list, total);
                }
