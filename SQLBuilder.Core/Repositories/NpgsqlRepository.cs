...
namespace SQLBuilder.Core.Repositories
{
    ...
    public class NpgsqlRepository : IRepository
    {
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string orderField, bool isAsc, int pageSize, int pageIndex) where T : class
        {
            ...
            if (Transaction?.Connection != null)
            {
                var multiQuery = Transaction.Connection.QueryMultiple($"DROP TABLE IF EXISTS TEMPORARY_{guid};CREATE TEMPORARY TABLE TEMPORARY_{guid} AS SELECT * FROM ({builder.Sql}) AS T;SELECT COUNT(1) AS Total FROM TEMPORARY_{guid};SELECT * FROM TEMPORARY_{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE TEMPORARY_{guid};", builder.DynamicParameters, Transaction, commandTimeout: CommandTimeout);
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"DROP TABLE IF EXISTS TEMPORARY_{guid};CREATE TEMPORARY TABLE TEMPORARY_{guid} AS SELECT * FROM ({builder.Sql}) AS T;SELECT COUNT(1) AS Total FROM TEMPORARY_{guid};SELECT * FROM TEMPORARY_{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE TEMPORARY_{guid};", builder.DynamicParameters, commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
        ...
        public (IEnumerable<T> list, long total) FindList<T>(string sql, object parameter, string orderField, bool isAsc, int pageSize, int pageIndex)
        {
            ...
            else
            {
                using (var dbConnection = Connection)
                {
                    var multiQuery = dbConnection.QueryMultiple($"DROP TABLE IF EXISTS TEMPORARY_{guid};CREATE TEMPORARY TABLE TEMPORARY_{guid} AS SELECT * FROM ({sql}) AS T;SELECT COUNT(1) AS Total FROM TEMPORARY_{guid};SELECT * FROM TEMPORARY_{guid} AS X {orderBy} LIMIT {pageSize} OFFSET {(pageSize * (pageIndex - 1))};DROP TABLE TEMPORARY_{guid};", parameter, commandTimeout: CommandTimeout);
                    var total = multiQuery?.ReadFirstOrDefault<long>() ?? 0;
                    var list = multiQuery?.Read<T>();
                    return (list, total);
                }
