...
namespace SQLBuilder.Core.Repositories
{
    ...
    public class NpgsqlRepository : IRepository, IDisposable
    {
        ...
        public NpgsqlRepository(string connString)
        {
            //判断是链接字符串，还是链接字符串名称
        ...
        {
            var result = 0;
            var builder = Sql.Delete<T>(DatabaseType.PostgreSQL).WithKey(keyValue);
            if (Transaction?.Connection != null)
            {
                result = Transaction.Connection.Execute(builder.Sql, builder.DynamicParameters, Transaction);
            }
        ...
        {
            var result = 0;
            var builder = Sql.Update<T>(DatabaseType: DatabaseType.PostgreSQL, isEnableNullValue: false).Where(predicate);
            if (Transaction?.Connection != null)
            {
                result = Transaction.Connection.Execute(builder.Sql, builder.DynamicParameters, Transaction);
            }
